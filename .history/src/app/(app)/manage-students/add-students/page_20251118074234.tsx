...
export default function AddStudentPage() {
  ...
  // Fetch schools list with design URLs
  ...
  // Memoize preview data for live preview updates
  const previewData = useMemo(() => ({
    ...
    formData.firstName, formData.lastName, formData.rollNumber,
    formData.fatherName, formData.photoUrl, selectedSchoolDesign
  ]);
  ...
  // Handle student photo upload and validation
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      Object.entries(formData).forEach(([key, value]) => {
        if (key !== 'photoUrl') formDataToSend.append(key, value.toString());
      });
  ...
  // Reset form to initial state, resetting selected school design as well
  const handleReset = () => {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-xl overflow-hidden">
          ...
          <div className="p-3 md:p-4 lg:p-5 border-b border-gray-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 xl:grid-cols-12 gap-4 md:gap-6 lg:gap-8">
              {/* Left Column - Photo & QR Code */}
              <div className="md:col-span-3 xl:col-span-3 flex flex-col gap-4 md:gap-6">
                {/* Photo Upload */}
                <div className="flex flex-col items-center">
                  ...
                  />
                  <p className="text-xs text-gray-700 mt-3 font-semibold">PHOTO (1.5 inches x 2 inches)</p>
                ...
                {/* QR Code */}
                <div className="flex flex-col items-center">
                  <label className="text-sm font-semibold text-gray-700 mb-3">Student QR Code</label>
                  <div className="w-40 h-40 border-2 border-dashed border-gray-300 rounded-xl flex items-center justify-center bg-white p-3 shadow-md">
                    <div className="w-full h-full bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center rounded-lg">
                      <span className="text-xs text-gray-500 text-center px-3">Generated after save</span>
                    </div>
                  </div>
                  <button
                    type="button"
              ...
              <div className="md:col-span-6 xl:col-span-6 space-y-3">
                {/* (Form fields for student details as in your original code, omitted here for brevity) */}
                {/* Make sure onChange calls handleChange */}
              </div>
              ...
              <div className="md:col-span-3 xl:col-span-3 space-y-4">
                <div className="flex flex-col items-center sticky top-6">
                  <h3 className="text-sm font-bold text-gray-800 mb-4">Your School ID Card Design</h3>
                    ...
                    {selectedSchoolDesign ? (
                      ...
                      <div className="text-center">
                        <div className="w-36 h-56 mx-auto bg-gradient-to-b from-purple-300 via-blue-300 to-blue-400 rounded-xl shadow-xl flex flex-col items-center justify-start p-4">
                          <div className="text-purple-800 font-bold text-xs mb-3">SCHOOL NAME</div>
                          ...
                          <div className="text-xs font-bold text-gray-900">{previewData.studentName}</div>
                          <div className="text-xs text-gray-800 mt-1.5">Roll: {previewData.rollNumber}</div>
                        </div>
                      </div>
                    )}
                  </div>
                  {!selectedSchoolDesign && (
                    <p className="text-xs text-gray-500 mt-3 text-center">Select a school to see the ID card design</p>
                  )}
                </div>
              </div>
            </div>
